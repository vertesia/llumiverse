 ...
 */
const TINY_PNG_B64 =
    'iVBORw0KGgoAAAANSUhEUgAAAAgAAAAICAYAAADED76LAAAADklEQVQI12P4' + 'z8BQDwADhQGAWjR9awAAAABJRU5ErkJggg==';
...
function makeEmbedContentDriver(embeddingsList: Array<{ values: number[]; statistics?: { tokenCount: number } }>) {
...
describe('generateVertexAiEmbeddings — text embedding (text-embedding-005)', () => {
    ...
    it('batches inputs with the same config in a single call', async () => {
        const { driver, mockEmbedContent } = makeEmbedContentDriver([{ values: FAKE_VECTOR }, { values: [0.4, 0.5] }]);
        const result = await generateVertexAiEmbeddings(driver, {
            inputs: [
...
describe('generateVertexAiEmbeddings — gemini-embedding-2 prefix model', () => {
    ...
    it("uses 'global' region client for gemini-embedding-2", async () => {
        ...
        const getClientSpy = vi
            .spyOn(driver, 'getGoogleGenAIClient')
            .mockReturnValue({ models: { embedContent: mockEmbedContent } } as never);
...
describe('generateLegacyMultimodalEmbeddings — multimodalembedding@001', () => {
    ...
    it('embeds an inline base64 image', async () => {
        const { driver, mockPredict } = makeLegacyDriver([{ imageEmbedding: FAKE_VECTOR }]);
    ...
    it('routes audio through the video path', async () => {
        const { driver, mockPredict } = makeLegacyDriver([{ videoEmbeddings: [{ embedding: FAKE_VECTOR }] }]);
    ...
    it('throws when prediction count mismatches input count', async () => {
        ...
        const { driver } = makeLegacyDriver([{ textEmbedding: FAKE_VECTOR }, { textEmbedding: [0.9, 0.8] }]);
