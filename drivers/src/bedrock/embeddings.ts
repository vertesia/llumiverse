...
async function invokeJson<R>(driver: BedrockDriver, modelId: string, body: unknown): Promise<R> {
...
async function buildNovaParams(
    input: EmbeddingInput,
): Promise<Omit<NovaSingleEmbeddingParams, 'embeddingPurpose' | 'embeddingDimension'>> {
    switch (input.type) {
        case 'text':
...
async function generateNovaEmbeddings(
    ...
    for (const input of options.inputs) {
        ...
        if (!item?.embedding) {
            throw new Error(
                `Nova embeddings response missing 'embeddings[0].embedding' for input type '${input.type}'`,
            );
        }
...
async function generateTitanEmbeddings(
    ...
    for (const input of options.inputs) {
        ...
        if (isImageModel) {
            ...
            items.push({
                outputs: [{ values: res.embedding, modality: input.type }],
                input_tokens: res.inputTextTokenCount,
            });
            ...
            items.push({
                outputs: [{ values: res.embedding, modality: 'text' }],
                input_tokens: res.inputTextTokenCount,
            });
            if (typeof res.inputTextTokenCount === 'number') totalTokens = (totalTokens ?? 0) + res.inputTextTokenCount;
        }
...
async function generateCohereEmbeddings(
    ...
    for (const [groupInputType, group] of textGroups) {
        ...
        if (res.embeddings.length !== group.length) {
            ...
            throw new Error(
                `Cohere returned ${res.embeddings.length} embeddings for ${group.length} texts (model ${modelId})`,
            );
        }
...
export async function generateBedrockEmbeddings(
    ...
    driver.logger.info(
        `[Bedrock] Generating embeddings with model ${modelId} for ${normalized.inputs.length} input(s)`,
    );
    try {
        ...
        if (
            ...
            typeof (error as { $metadata?: unknown }).$metadata === 'undefined' &&
