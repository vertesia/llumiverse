...
describe('generateBedrockEmbeddings — Nova', () => {
    ...
    it('throws when response has no embeddings array', async () => {
        ...
        await expect(generateBedrockEmbeddings(driver, { inputs: [{ type: 'text', text: 'x' }] })).rejects.toThrow(
            "Nova embeddings response missing 'embeddings[0].embedding'",
        );
    });
});
...
describe('generateBedrockEmbeddings — Nova S3 URL passthrough', () => {
    ...
    it('converts path-style HTTPS S3 URL to correct s3:// URI (Nova audio)', async () => {
        ...
        const { driver, executor } = makeDriver({ embeddings: [{ embeddingType: 'AUDIO', embedding: FAKE_VECTOR }] });
        const ds = new URLDataSource(
            'audio.mp3',
            'audio/mpeg',
            'https://s3.us-east-1.amazonaws.com/my-bucket/audio.mp3',
        );
    ...
    it('converts virtual-hosted HTTPS S3 URL to correct s3:// URI (Nova video)', async () => {
        ...
        const ds = new URLDataSource(
            'vid.mp4',
            'video/mp4',
            'https://my-bucket.s3.us-east-1.amazonaws.com/folder/vid.mp4',
        );
